...
var QLearn = function() {
    ...
    this.addState = function(currentState, action, rewardFromNextState, nextState) {
        ...
        for(var value in this.transitionMatrix[nextState]) {
            ...
            if(this.transitionMatrix[nextState][value] < minOfNextState) {
                minOfNextState = this.transitionMatrix[nextState][value];
            }
        }

        // console.log(currentState, action, this.rewardTable[nextState], nextState, this.learningRate, maxOfNextState);

        this.transitionMatrix[currentState][action] = this.rewardTable[nextState] + (this.learningRate*maxOfNextState) + (this.learningRate*minOfNextState);
    ...
    this.getBestPolicyOne = function(state) {
        var actionPool = this.transitionMatrix[state];
        var action = 'undefined';
        var maxScore = -99999999;
